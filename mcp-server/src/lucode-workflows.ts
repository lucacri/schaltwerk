export interface LucodeWorkflowRegistryEntry {
  name: string
  title: string
  description: string
  resource_uri: string
...
const CODEX_CONSOLIDATE_SKILL_PATH = '.codex/skills/consolidate/SKILL.md'
const OPENCODE_CONSOLIDATE_COMMAND_PATH = '.opencode/commands/consolidate.md'

interface LucodeWorkflowDefinition extends LucodeWorkflowRegistryEntry {
  markdown: string
}

const consolidateMarkdown = `# Consolidate Lucode Sessions
...
const workflowDefinitions: LucodeWorkflowDefinition[] = [
  {
    name: 'consolidate',
    title: 'Consolidate Lucode Sessions',
    description: 'Compare parallel Lucode sessions for the same spec and promote the winning consolidation result.',
    resource_uri: 'lucode://skills/consolidate',
    native_entrypoints: {
      shared: '.agents/skills/consolidate/SKILL.md',
      claude: 'claude-plugin/commands/consolidate.md',
      codex: CODEX_CONSOLIDATE_SKILL_PATH,
      opencode: OPENCODE_CONSOLIDATE_COMMAND_PATH,
    },
    markdown: consolidateMarkdown,
  },
]

function getWorkflowDefinition(name: string): LucodeWorkflowDefinition {
  const workflow = workflowDefinitions.find(candidate => candidate.name === name)
  if (!workflow) {
    throw new Error(`Unknown Lucode workflow: ${name}`)
  }

  return workflow
}

export function listLucodeWorkflows(): LucodeWorkflowRegistryEntry[] {
  return workflowDefinitions.map(workflow => ({
    name: workflow.name,
    title: workflow.title,
    description: workflow.description,
    resource_uri: workflow.resource_uri,
    native_entrypoints: workflow.native_entrypoints,
  }))
}

export function readLucodeWorkflowMarkdown(name: string): string {
  return getWorkflowDefinition(name).markdown
}

export function renderClaudeSkillFile(name: string): string {
  const workflow = getWorkflowDefinition(name)
  return `---\nname: ${workflow.name}\ndescription: ${workflow.description}\n---\n\n${workflow.markdown}\n`
}

export function renderClaudeCommandFile(name: string): string {
  const workflow = getWorkflowDefinition(name)
  return `---\ndescription: ${workflow.description}\n---\n\n${workflow.markdown}\n`
}

export function renderCodexSkillFile(name: string): string {
  return renderSharedAgentSkillFile(name)
}

export function renderSharedAgentSkillFile(name: string): string {
  const workflow = getWorkflowDefinition(name)
  return `---\nname: ${workflow.name}\ndescription: ${workflow.description}\n---\n\n${workflow.markdown}\n`
}

export function renderOpenCodeCommandFile(name: string): string {
  const workflow = getWorkflowDefinition(name)
  return `---\ndescription: ${workflow.description}\n---\n\n${workflow.markdown}\n`
}

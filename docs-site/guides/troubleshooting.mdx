---
title: Troubleshooting
description: 'Common issues and solutions for Lucode'
---

This guide covers common issues you might encounter and how to resolve them.

> **Tip:** Examples use `bun` commands. Replace them with the npm equivalents (`npm run …`, `npm install`, etc.) if needed.

## Installation Issues

<AccordionGroup>
  <Accordion title="Lucode won't open on first launch">
    **Symptom:** macOS blocks the app with "can't be opened because it is from an unidentified developer"

    **Solution:**
    1. Go to **System Settings → Privacy & Security**
    2. Scroll down to find: "Lucode was blocked"
    3. Click **Open Anyway**
    4. Launch Lucode again and click **Open**

    Only needed once - macOS will remember your choice.
  </Accordion>

  <Accordion title="App is damaged and can't be opened">
    **Symptom:** Error message about damaged application

    **Solution:**
    Remove the quarantine attribute:
    ```bash
    xattr -cr /Applications/Lucode.app
    ```

    Then try launching again.
  </Accordion>

</AccordionGroup>

## Task & Slot Session Issues

<AccordionGroup>
  <Accordion title="Can't create new task or advance a stage">
    **Symptom:** Error when trying to create a task or start a stage run

    **Possible Causes & Solutions:**

    **1. Not in a Git repository:**
    ```bash
    git status
    # If error: "not a git repository"
    git init
    ```

    **2. Working directory not clean:**
    ```bash
    git status
    # Commit or stash changes
    git stash
    ```

    **3. Worktree directory exists:**
    ```bash
    ls .lucode/worktrees/
    # If a stale task worktree directory exists, cancel the task first
    ```

    **4. Permission issues:**
    ```bash
    # Check permissions
    ls -la .lucode/
    # Fix if needed
    chmod -R u+w .lucode/
    ```
  </Accordion>

  <Accordion title="Task stuck in 'Creating' state">
    **Symptom:** Task shows "Creating..." but never completes

    **Solution:**
    1. Check Lucode logs:
       ```bash
       tail -f ~/Library/Application\ Support/lucode/logs/lucode-*.log
       ```
    2. Look for errors in the log
    3. Common causes:
       - Git worktree creation failed
       - Disk space full
       - Permission denied
    4. Try cancelling and recreating the task
  </Accordion>

  <Accordion title="Terminal won't start">
    **Symptom:** Slot session opened but terminals are blank or frozen

    **Solution:**
    1. Check terminal permissions:
       - **System Settings → Privacy & Security → Developer Tools**
       - Add Terminal.app
    2. Restart Lucode
    3. Try selecting the task or slot session again
    4. Check logs for PTY errors:
       ```bash
       tail -f ~/Library/Application\ Support/lucode/logs/lucode-*.log | grep PTY
       ```
  </Accordion>

  <Accordion title="Can't switch between tasks">
    **Symptom:** Keyboard shortcuts don't work or the active task doesn't change

    **Solution:**
    1. Check if another app is intercepting shortcuts:
       - **System Settings → Keyboard → Keyboard Shortcuts**
       - Look for conflicts with `⌘1-9`
    2. Try clicking the task in the sidebar instead
    3. Restart Lucode
  </Accordion>
</AccordionGroup>

## Agent Issues

<AccordionGroup>
  <Accordion title="Agent won't start">
    **Symptom:** Top terminal shows error or nothing happens

    **Possible Causes & Solutions:**

    **1. Agent binary not found:**
    - Open Settings
    - Go to **Agent Configuration**
    - Verify binary path is correct
    - Try: `which claude`, `which codex`, or `which droid` in terminal

    **2. Agent binary not executable:**
    ```bash
    chmod +x /path/to/agent/binary
    ```

    **3. Environment variables missing:**
    - Check Settings → Agent Configuration → Environment Variables
    - Add required keys (e.g., `ANTHROPIC_API_KEY`)

    **4. Setup script failed:**
    - Check Settings → Run & Environment → Worktree Setup Script
    - Look for errors in top terminal output
    - Try running script manually in worktree
  </Accordion>

  <Accordion title="Agent crashes immediately">
    **Symptom:** Agent starts but exits right away

    **Solution:**
    1. Check agent terminal output for errors
    2. Common issues:
       - Invalid API key
       - Network connectivity
       - Incompatible CLI arguments
    3. Try running agent manually:
       ```bash
       cd .lucode/worktrees/session-name/
       claude  # or your agent command
       ```
    4. Check agent-specific logs
  </Accordion>

  <Accordion title="Agent fails with 'ReadableStream is not defined' or similar errors">
    **Symptom:** Error like "ReferenceError: ReadableStream is not defined" when starting agent

    **Solution:**
    This occurs when using Node version managers (fnm, nvm, n) that set paths via shell config files. Lucode spawns agents without loading `.zshrc`/`.bashrc`, so falls back to system Node, which may be outdated.

    Install Node via Homebrew to set a global default:
    ```bash
    brew install node
    ```

    Verify system Node version (what Lucode uses):
    ```bash
    fnm use system && node --version  # For fnm users
    nvm use system && node --version  # For nvm users
    # Should show v18+
    ```
  </Accordion>

  <Accordion title="Agent hangs or becomes unresponsive">
    **Symptom:** Agent stops responding, no output

    **Solution:**
    1. Check agent terminal - might be waiting for input
    2. Press `Ctrl+C` to interrupt
    3. Click **Restart** in the task / slot session toolbar
    4. If persists, cancel the task and create a new one
  </Accordion>
</AccordionGroup>

## Git Issues

<AccordionGroup>
  <Accordion title="Merge conflicts when merging to main">
    **Symptom:** Can't merge a task's branch back to main due to conflicts

    **Solution:**
    1. **Option 1: Merge main into the task worktree first**
       ```bash
       cd .lucode/worktrees/task-name/
       git fetch origin main
       git merge origin/main
       # Resolve conflicts
       git add .
       git commit
       ```

    2. **Option 2: Cherry-pick specific commits**
       ```bash
       git checkout main
       git cherry-pick <commit-hash>
       ```

    3. **Option 3: Manual merge**
       - Copy changes you want
       - Apply manually to main branch
       - Cancel the task when done
  </Accordion>

  <Accordion title="Worktree directory already exists">
    **Symptom:** Error: "worktree already exists" when creating a task

    **Solution:**
    1. Check if worktree is actually in use:
       ```bash
       git worktree list
       ```
    2. If the task was cancelled but worktree remains:
       ```bash
       git worktree remove .lucode/worktrees/task-name
       ```
    3. If worktree is locked:
       ```bash
       git worktree remove --force .lucode/worktrees/task-name
       ```
    4. Retry creating the task
  </Accordion>

  <Accordion title="Can't delete branch after cancelling a task">
    **Symptom:** Task cancelled but branch still exists

    **Solution:**
    Cancelling a task deletes its worktree and branch automatically. If a branch remains, it likely wasn't cancelled cleanly. Remove it manually:

    ```bash
    git branch -D lucode/task-name
    ```

    To delete from remote (if pushed):
    ```bash
    git push origin --delete lucode/task-name
    ```
  </Accordion>
</AccordionGroup>

## Performance Issues

<AccordionGroup>
  <Accordion title="Lucode is slow or laggy">
    **Symptom:** UI is unresponsive, switching sessions is slow

    **Possible Causes & Solutions:**

    **1. Too many slot sessions running:**
    - Each slot session uses 3 OS processes
    - Close unused slot sessions
    - Don't run more than 5-10 slot sessions simultaneously

    **2. Large worktrees:**
    - Big repositories take longer to create worktrees
    - Use `.gitignore` to exclude unnecessary files
    - Clean up `node_modules` in worktrees

    **3. System resources:**
    ```bash
    # Check CPU/memory usage
    top -o cpu
    # Look for Lucode processes
    ```

    **4. Database size:**
    ```bash
    # Check database size
    du -sh ~/Library/Application\ Support/lucode/
    # If very large, consider cleaning old projects
    ```
  </Accordion>

  <Accordion title="Diff view is slow to load">
    **Symptom:** File diffs take long time to appear

    **Solution:**
    1. Large files slow down diff rendering
    2. Exclude large binary files from worktrees
    3. Use `.gitattributes` to mark files as binary
    4. Consider breaking up large files
  </Accordion>
</AccordionGroup>

## MCP Server Issues

<AccordionGroup>
  <Accordion title="Claude Code can't find MCP server">
    **Symptom:** MCP tools not available in Claude Code

    **Solution:**
    1. Verify MCP server is built:
       ```bash
       ls mcp-server/build/lucode-mcp-server.js
       ```
    2. Check configuration:
       ```bash
       cat .claude.json
       # or
       claude mcp list
       ```
    3. Ensure path is absolute, not relative
    4. Restart orchestrator in Lucode Settings
  </Accordion>

  <Accordion title="MCP operations fail">
    **Symptom:** `lucode_create` or other MCP tools error

    **Solution:**
    1. Check Lucode is running
    2. Verify project is loaded in Lucode
    3. Check database exists:
       ```bash
       ls ~/Library/Application\ Support/lucode/*/database.db
       ```
    4. Look for errors in MCP server logs
    5. Try rebuilding MCP server:
       ```bash
       cd mcp-server && bun run build
       ```
  </Accordion>
</AccordionGroup>

## Database Issues

<AccordionGroup>
  <Accordion title="Database locked or corrupted">
    **Symptom:** Errors about database being locked or unable to open

    **Solution:**
    1. **Close Lucode completely**
    2. **Check for zombie processes:**
       ```bash
       ps aux | grep Lucode
       # Kill any remaining processes
       kill -9 <PID>
       ```
    3. **Backup database:**
       ```bash
       cp ~/Library/Application\ Support/lucode/project-name/database.db \
          ~/Desktop/lucode-backup.db
       ```
    4. **Try SQLite repair:**
       ```bash
       sqlite3 ~/Library/Application\ Support/lucode/project-name/database.db
       # In SQLite prompt:
       PRAGMA integrity_check;
       .exit
       ```
    5. **Last resort - delete and recreate:**
       ```bash
       rm ~/Library/Application\ Support/lucode/project-name/database.db
       # Restart Lucode - will create fresh database
       ```
  </Accordion>
</AccordionGroup>

## Network Issues

<AccordionGroup>
  <Accordion title="MCP REST API not accessible">
    **Symptom:** Can't connect to `localhost:8547`

    **Solution:**
    1. Check if Lucode is running
    2. Verify project is loaded
    3. Check if port is available:
       ```bash
       lsof -i :8547
       # or for project-specific port
       lsof -i :854X  # X depends on project hash
       ```
    4. Try restarting Lucode
    5. Check firewall settings
  </Accordion>

  <Accordion title="Agent can't access internet">
    **Symptom:** Agent errors related to network/API calls

    **Solution:**
    1. Check system internet connection
    2. Verify API keys are correct
    3. Check for proxy settings
    4. Try agent manually in terminal
    5. Look for firewall blocking the agent
  </Accordion>
</AccordionGroup>

## Logs and Debugging

### Where to Find Logs

<CodeGroup>
```bash Application Logs
~/Library/Application Support/lucode/logs/lucode-*.log
```

```bash Tail Logs in Real-Time
tail -f ~/Library/Application\ Support/lucode/logs/lucode-*.log
```

```bash Filter for Errors
grep -i error ~/Library/Application\ Support/lucode/logs/lucode-*.log
```

```bash Task-Specific Logs
# Check terminal output in the task or slot session itself
# Or look for task / session id in main logs
grep "task-name" ~/Library/Application\ Support/lucode/logs/*.log
```
</CodeGroup>

### Debug Mode

Run Lucode with debug logging:

```bash
RUST_LOG=lucode=debug bun run tauri:dev
```

Or for maximum verbosity:

```bash
RUST_LOG=trace bun run tauri:dev
```

## Getting Help

If you can't resolve an issue:

<Steps>
  <Step title="Check logs">
    Look at Lucode logs for error messages
  </Step>

  <Step title="Search existing issues">
    Check [GitHub Issues](https://github.com/lucacri/lucode/issues)
  </Step>

  <Step title="Create an issue">
    If not found, open a new issue with:
    - Steps to reproduce
    - Expected behavior
    - Actual behavior
    - Relevant log excerpts
    - macOS version
    - Lucode version
  </Step>

  <Step title="Workaround">
    While waiting for fix:
    - Try manual git operations
    - Use terminal directly
    - Restart Lucode
    - Close and reopen project
  </Step>
</Steps>

## Preventive Measures

<CardGroup cols={2}>
  <Card title="Regular Cleanup" icon="broom">
    - Cancel merged tasks
    - Remove old worktrees
    - Clean up database periodically
  </Card>

  <Card title="Keep Updated" icon="arrow-up">
    ```bash
    just install-fast
    ```
    Rebuild regularly for bug fixes
  </Card>

  <Card title="Backup Important Work" icon="floppy-disk">
    - Commit work in task worktrees regularly
    - Push branches to remote
    - Don't rely on local-only tasks
  </Card>

  <Card title="Monitor Resources" icon="gauge">
    - Don't run too many slot sessions
    - Close unused slot sessions
    - Watch system resource usage
  </Card>
</CardGroup>

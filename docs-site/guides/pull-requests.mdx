...
By default, Lucode uses the session branch as the PR head branch. Enable **Use custom PR branch name** to push the PR to a different branch instead (useful for avoiding conflicts or keeping the session branch untouched).
...
When a session is linked to a PR, Lucode can fetch GitHub **review comments** (inline comments on files/lines):
...
If you have MCP enabled, agents can request that Lucode open the PR modal for a session:

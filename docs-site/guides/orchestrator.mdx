...
<Info>
  **Requires MCP:** The orchestrator agent must be able to call `lucode_current_spec_update` to edit specs. Enable MCP in **Settings → Agent Configuration** for your agent (Claude Code, Codex, OpenCode, or Gemini).
</Info>
...
<Steps>
  ...
  <Step title="Iterative refinement">
    ...
    - Update the spec with `lucode_current_spec_update`

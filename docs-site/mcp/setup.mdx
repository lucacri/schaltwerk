...
<Steps>
  ...
  <Step title="Enable the MCP toggle">
    In the **MCP Server Configuration** panel, turn on **Enable MCP**. Lucode writes the necessary `.mcp.json` entry using the embedded server binary.
...
<AccordionGroup>
  ...
  <Accordion title="Need to remove MCP configuration">
    Turn off the **Enable MCP** toggle. Lucode removes the entry from `.mcp.json` (or the agent’s global config).
  </Accordion>
</AccordionGroup>

...
describe('generationPrompts', () => {
  ...
  it('falls back to frontend defaults when prompt loading fails', async () => {
    ...
    expect(prompts.consolidation_prompt).toContain('mermaid')
    expect(prompts.consolidation_prompt).toContain('when it makes sense')
    expect(prompts.consolidation_prompt).toContain('architecture')
    expect(prompts.consolidation_prompt).toContain('data or control flow')
    expect(prompts.consolidation_prompt).toContain('state machines')
    expect(prompts.consolidation_prompt).toContain('sequence of events')
    expect(prompts.consolidation_prompt).toContain('base_session_id')
    expect(prompts.consolidation_prompt).not.toContain('winner_session_id')
    expect(prompts.plan_candidate_prompt_template).toContain('mermaid')
    expect(prompts.plan_candidate_prompt_template).toContain('when it makes sense')
    expect(prompts.plan_candidate_prompt_template).toContain('{SPEC_ID}')
    expect(prompts.plan_candidate_prompt_template).toContain('{BASE_SPEC_CONTENT}')
    expect(prompts.plan_judge_prompt_template).toContain('mermaid')
    expect(prompts.plan_judge_prompt_template).toContain('when it makes sense')
    expect(prompts.plan_judge_prompt_template).toContain('{CANDIDATES_BLOCK}')
    expect(prompts.plan_judge_prompt_template).toContain('{SOURCE_SESSIONS_BLOCK}')
    expect(prompts.judge_prompt_template).toContain('mermaid')
    expect(prompts.judge_prompt_template).toContain('when it makes sense')
    expect(prompts.judge_prompt_template).toContain('{CANDIDATE_COUNT}')
    expect(prompts.judge_prompt_template).toContain('synthesis judge')

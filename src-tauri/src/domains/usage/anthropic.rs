...
pub fn parse_usage_response(json: &serde_json::Value) -> Result<UsageSnapshot, String> {
    ...
    let session_raw = session
        .get("usage_percent")
        .and_then(|v| v.as_u64())
        .ok_or("Missing session.usage_percent")?;
    let session_percent = u8::try_from(session_raw.min(100))
        .unwrap_or(100);

    let weekly_raw = weekly
        .get("usage_percent")
        .and_then(|v| v.as_u64())
        .ok_or("Missing weekly.usage_percent")?;
    let weekly_percent = u8::try_from(weekly_raw.min(100))
        .unwrap_or(100);
...
mod tests {
    ...
    #[test]
    fn test_parse_usage_response_clamps_overflow() {
        let json = serde_json::json!({
            "session": { "usage_percent": 300 },
            "weekly": { "usage_percent": 999 }
        });
        let snapshot = parse_usage_response(&json).unwrap();
        assert_eq!(snapshot.session_percent, 100);
        assert_eq!(snapshot.weekly_percent, 100);
    }
}

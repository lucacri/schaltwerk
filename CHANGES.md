...
## Improve Plan: remove `candidate_count` parameter

The legacy `candidate_count` knob (default 2, clamp 1..6) on Improve Plan rounds was a pre-preset workaround for "spawn N copies of the same agent". It is now removed from every API surface: the HTTP body on `POST /api/specs/{name}/improve-plan`, the `schaltwerk_core_start_improve_plan_round` Tauri command, the MCP bridge options, and the `lucode_improve_plan` MCP tool schema. Improve Plan now always creates exactly one candidate session per round (`-v1`) using the supplied `agent_type` (or the project default). Migration path: when the successor preset-aware Improve Plan lands, use a preset whose slot list contains the desired agents — a preset like `[claude, claude]` replaces the old `candidate_count: 2` behaviour.

- `src-tauri/src/mcp_api.rs`: drops `candidate_count` from `ImprovePlanRoundRequest` and `StartImprovePlanRoundParams` (plus the `From` impl), removes the `let candidate_count = params.candidate_count.unwrap_or(2).clamp(1, 6)` line, and replaces the `0..candidate_count` loop in `create_improve_plan_round_start_context` with a single-candidate block pinned to `-v1` / `version_number = 1`.
- `src-tauri/src/commands/schaltwerk_core.rs`: removes the `candidate_count: Option<usize>` parameter from `schaltwerk_core_start_improve_plan_round` and its forwarding into `StartImprovePlanRoundParams`.
- `mcp-server/src/lucode-bridge.ts`: `startImprovePlanRound` no longer accepts `candidateCount` or forwards `candidate_count` in the JSON body.
- `mcp-server/src/lucode-mcp-server.ts`: `LucodeImprovePlanArgs` drops `candidate_count`; the tool input schema removes the property; and the handler stops mapping it into the bridge call. Because the tool schema sets `additionalProperties: false`, MCP clients still sending `candidate_count` will now get a validation error — this is a hard removal (solo-macOS app, no deprecation window).
- Tests: `mcp_api.rs::start_improve_plan_round_produces_single_candidate_by_default` locks in the new one-candidate default; `start_improve_plan_round_respects_read_lock_during_launch` drops the `candidate_count: Some(1)` override; `start_improve_plan_round_rolls_back_all_state_when_candidate_launch_fails` is reworked so the single launch fails and rollback wipes the spec link, round row, and candidate session. Bridge and tool-handler TS fixtures drop `candidate_count`/`candidateCount`.
